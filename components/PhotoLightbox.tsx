...
type Props = {
  ...
  onClose: () => void;
  /** Extra controls rendered over the bottom of the photo, below the
   * thumbnail strip — e.g. a variant or color pill row, so a shopper can
   * switch either without closing the viewer. */
    ...
    >
      ...
      >
        ...
        <img
          ...
          alt=""
          draggable={false}
          onDoubleClick={() => setZoom((z) => (z > 1 ? 1 : ZOOM_DOUBLE_CLICK))}
      ...
      <button
        ...
        aria-label="Close"
        onClick={onClose}
        className="absolute top-5 right-5 z-20 w-10 h-10 rounded-full bg-white/10 text-white flex items-center justify-center hover:bg-white/20"
      >
      ...
      <div
        className="absolute top-5 left-5 z-20 flex items-center gap-1 bg-white/10 rounded-full p-1"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          type="button"
      ...
      {(hasGallery || extraControls) && (
        <div
          className="absolute bottom-0 left-0 right-0 z-20 pt-16 pb-6 px-6 bg-gradient-to-t from-black/80 via-black/40 to-transparent"
          onClick={(e) => e.stopPropagation()}
        >
          {hasGallery && (
...
function LightboxArrow({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`absolute top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full flex items-center justify-center transition-colors bg-white/10 text-white hover:bg-white/20 ${
