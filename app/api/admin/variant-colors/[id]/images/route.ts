...
export async function POST(
  ...
  const { count } = await supabase
    .from("variant_color_images")
    .select("id", { count: "exact", head: true })
    .eq("variant_color_id", id);

  const { data, error } = await supabase
    .from("variant_color_images")
    .insert({ variant_color_id: id, image_url: imageUrl, sort_order: count ?? 0 })
...
export async function PATCH(
  ...
  const results = await Promise.all(
    order.map((imageId, index) =>
      supabase.from("variant_color_images").update({ sort_order: index }).eq("id", imageId).eq("variant_color_id", id)
    )
  );
